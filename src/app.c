...
static void handle_read(const Command *cmd)
{
    ...
    if (cmd->start_line > 0 && cmd->end_line > 0) {
        snprintf(msg, sizeof(msg), "Tool_Use [read_file] file=%s lines=%d-%d",cmd->file, cmd->start_line, cmd->end_line);
    } else if (cmd->lines > 0) {
        snprintf(msg, sizeof(msg), "Tool_Use [read_file] file=%s lines=%d",cmd->file, cmd->lines);
    } else {
...
static void handle_write(const Command *cmd)
{
    ...
    tui_add_chat_line(msg, 0);
    tui_add_chat_line("", 0);
    int result = tool_write_file(cmd->file, cmd->content,errmsg, sizeof(errmsg));
    if (result == 0) {
